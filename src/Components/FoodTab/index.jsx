...
const MobileTabs= ()=> {
    ...
    return(
        <>
            <div className= "lg:hidden bg-white p-3 shadow-lg fixed bottom-0 z-10 w-full flex items-center justify-between md: justify-evenly text-gray-500 border">
                
                {allTypes.map((items)=> (
                    <Link to= {`/${items.id}`}>
                        <div className= { type=== items.id ? "flex flex-col relative items-center text-xl text-zomato-400" :
                        "flex flex-col items-center text-xl"}>
                        <div className= { type=== items.id && "absolute -top-4 w-full h-2 border-t-2 border-zomato-400"} />
...
const LargeTabs= ()=> {
    ...
    return(
        <>
          <div className= "hidden lg:flex container px-20 mx-auto my-8 gap-14">
              {allTypes.map((items)=> (
                  <Link to= {`/${items.id}`}>
                        ...
                        <div className= {classNames(
                            "w-16 h-16 p-4 bg-gray-100 rounded-full p-4", 
                            {
                        ...
                        <h3 className={type=== items.id ? "text-xl text-zomato-400" : "text-xl text-gray-700 font-semibold"} >
                        {items.name}</h3>
                    </div>
                </Link>
              ))}
